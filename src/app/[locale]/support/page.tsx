...
export default function SupportPage({ params: { locale } }: { params: { locale: Locale } }) {
    ...
    useEffect(() => {
        // Use real-time listener for social media settings
        const docRef = doc(db, 'siteSettings', 'socialMedia');
        const unsubscribe = onSnapshot(docRef, 
            (docSnap) => {
                if (docSnap.exists()) {
                    setSocialSettings(docSnap.data());
                } else {
            ...
            (error) => {
                console.error('Error listening to social media settings:', error);
    ...
    if (socialSettings?.facebook?.enabled && socialSettings?.facebook?.url) {
        socialMedia.push({
            ...
            url: socialSettings.facebook.url
        });
    }
    
    if (socialSettings?.instagram?.enabled && socialSettings?.instagram?.url) {
        socialMedia.push({
            ...
            url: socialSettings.instagram.url
        });
    }
    
    if (socialSettings?.twitter?.enabled && socialSettings?.twitter?.url) {
        socialMedia.push({
            ...
            url: socialSettings.twitter.url
        });
    }
    ...
    return (
        <div className="w-full pt-16">
            ...
            {/* Social Media Section - Only show if there are enabled social media */}
