...
import { Plus, Edit2, Trash2, Save, X, Upload, Star, Package } from 'lucide-react';
...
interface FeaturedProductsTabProps {
    currentLocale: string;
    showToast: (message: string, type: 'success' | 'error' | 'info') => void;
    t: any;
}

export function FeaturedProductsTab({ currentLocale, showToast, t }: FeaturedProductsTabProps) {
    ...
    const loadProducts = async () => {
        ...
        try {
            ...
            querySnapshot.forEach((doc) => {
                const data = doc.data();
                if (!data.isDeleted) {
                    items.push({ id: doc.id, ...data } as FeaturedProduct);
                }
            });
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-gray-500">
                    ℹ️ {
                        currentLocale === 'ko' ? '홈 / 제품 > 추천 제품' :
                        currentLocale === 'en' ? 'Home / Products > Featured Products' :
                        currentLocale === 'id' ? 'Beranda / Produk > Produk Unggulan' :
                        currentLocale === 'zh' ? '主页 / 产品 > 推荐产品' :
                        currentLocale === 'ja' ? 'ホーム / 製品 > おすすめ製品' :
                        currentLocale === 'ar' ? 'الصفحة الرئيسية / المنتجات > المنتجات المميزة' :
            ...
            {editingItem && (
                ...
                >
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                            {currentLocale === 'ko' ? '제품 링크' :
                             currentLocale === 'en' ? 'Product Link' :
                             currentLocale === 'id' ? 'Tautan Produk' :
                             currentLocale === 'zh' ? '产品链接' :
                             currentLocale === 'ja' ? '製品リンク' :
                             currentLocale === 'ar' ? 'رابط المنتج' :
                             'Product Link'}
                        </label>
                        <input
                            type="text"
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-2">
                                        <Package className="text-green-500 flex-shrink-0" size={20} />
                                    ...
                                    {item.productLink && (
                                        <div className="text-xs text-gray-500 mb-2">
                                            <span className="font-medium">
                                                {currentLocale === 'ko' ? '링크' : 'Link'}:
                                            </span>
                                            <a href={item.productLink} target="_blank" rel="noopener noreferrer" className="ml-1 text-blue-500 hover:underline">
                                                {item.productLink}
                                            </a>
                                        </div>
                                    )}
