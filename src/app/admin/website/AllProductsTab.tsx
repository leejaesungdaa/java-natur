...
export function AllProductsTab({ currentLocale, showToast, t }: AllProductsTabProps) {
    ...
    const loadProducts = async () => {
        ...
        try {
            ...
            querySnapshot.forEach((doc) => {
                const data = doc.data();
                if (!data.isDeleted && !data.featured) {
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-gray-500">
                    ℹ️ {
                        currentLocale === 'ko' ? '제품 > 모든 제품' :
                        currentLocale === 'en' ? 'Products > All Products' :
                        currentLocale === 'id' ? 'Produk > Semua Produk' :
                        currentLocale === 'zh' ? '产品 > 所有产品' :
                        currentLocale === 'ja' ? '製品 > すべての製品' :
                        currentLocale === 'ar' ? 'المنتجات > جميع المنتجات' :
