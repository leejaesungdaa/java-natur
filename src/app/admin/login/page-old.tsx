...
export default function AdminLogin() {
    ...
    useEffect(() => {
        ...
        const savedLocale = locale || getAdminLanguage() || syncLanguageFromWebsite();
        setCurrentLocale(savedLocale);
        const translations = getAdminTranslation(savedLocale);
        setT(translations);
    ...
    const handleLocaleChange = (newLocale: string) => {
        ...
        updateToastMessages((message) => {
            ...
            for (const key of errorKeys) {
                if (message === (oldT.auth.login.errors as any)[key]) {
                    return (newT.auth.login.errors as any)[key];
                }
            }
            ...
            for (const key of validationKeys) {
                if (key in oldT.auth.login.validation) {
                    if (message === (oldT.auth.login.validation as any)[key]) {
                        return (newT.auth.login.validation as any)[key];
                    }
                }
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!credentials.username.trim()) {
            showToast(t.auth.login.errors.usernameRequired, 'error');
            return;
        }
        ...
        try {
            const adminEmail = `${credentials.username}@admin.naturjava.local`;
            
            // 먼저 Firebase Auth로 인증 시도
            let userCredential;
            try {
                ...
                if (authError.code === 'auth/wrong-password' || 
                    ...
                    showToast(t.auth.login.errors.authFailed, 'error');
                }
                setLoading(false);
                return;
            }
            
            // 인증 성공 후 Firestore에서 사용자 정보 가져오기
            let adminData;
            let adminId = userCredential.user.uid;
            ...
            try {
                ...
                // Firestore 읽기 실패 시 기본 승인 상태로 처리
                adminData = { status: 1, username: credentials.username };
            }
            
            const userStatus = typeof adminData.status === 'string' ? parseInt(adminData.status) : adminData.status;
            ...
            try {
                await updateDoc(doc(db, 'admins', adminId), {
                    lastLogin: getIndonesiaTime()
                });
            } catch (error) {
                // Silently handle lastLogin update failure
            }
                        ...
                        <div>
                            <label className="flex items-center gap-2 text-gray-300 text-sm cursor-pointer">
                                <input
                                    ...
                                    className="w-4 h-4 rounded border-white/20 bg-white/10 text-green-500 focus:ring-green-500 focus:ring-offset-0 focus:ring-2"
                                />
                                <span>{t.auth.login.rememberMe || 'Remember username'}</span>
                            </label>
                        </div>
